...
public class MyHttpPostRequestEncoder implements ChunkedInput<HttpContent>{
	 ...
	 * @throws ErrorDataEncoderException 
	 */
	public void addBodyStreamUpload(String name,FileChunk fileChunk, String contentType, boolean isText) {
		  if (name == null) {
	            throw new NullPointerException("name");
	        }
	        if (fileChunk == null) {
	            throw new NullPointerException("fileChunk");
	        }
	        String scontentType = contentType;
	        String contentTransferEncoding = null;
	        if (contentType == null) {
	        ...
	        byte[] chunks=fileChunk.getBytes();
	        FileUpload fileUpload = factory.createFileUpload(request, name, String.valueOf(fileChunk.getChunk()), scontentType,
	                contentTransferEncoding, null, chunks.length);
	        factory.createAttribute(request, "chunks",String.valueOf(fileChunk.getChunk()));
	        factory.createAttribute(request, "checksum",fileChunk.getHash());
	        try {
	            fileUpload.setContent(wrappedBuffer(fileChunk.getBytes()));
	            addBodyHttpData(fileUpload);
	        } catch (Exception e) {
